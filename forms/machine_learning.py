...
def main():
    ...
    with open('data/metadata/model_params.json', 'w') as json_file:
        ...
        if pred_type == "Regression":
            ...
            if dt_r2 > lr_r2:
                # save decision tree 
                joblib.dump(dt_model, 'sample/metadata/model_reg.sav')
            else:
                joblib.dump(lr_model, 'sample/metadata/model_reg.sav')
        ...
        if pred_type == "Classification":
            ...
            if dtc_acc > lc_acc:
                # save decision tree 
                joblib.dump(dtc_model, 'sample/metadata/model_classification.sav')
            else:
                joblib.dump(lc_model, 'sample/metadata/model_classification.sav')
